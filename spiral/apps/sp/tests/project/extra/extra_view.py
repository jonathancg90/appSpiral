...
class ExtraViewTest(TestCase):
    ...
    def test_save_casting_method(self):
        ...
        casting = extra_save_process.save_extra(project)
        extra_save_process.save_detail_model_extra(casting)

        self.assertEqual(len(Extras.objects.all()), 1)
        self.assertEqual(len(ExtrasDetailModel.objects.filter(casting=casting)), 2)